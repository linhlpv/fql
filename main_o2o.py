...
flags.DEFINE_string('project_name', "FQL_auxilary_dev", 'Wandb project name.')
flags.DEFINE_string('exp_notes', None, 'note for experiments.')

config_flags.DEFINE_config_file('agent', 'agents/fql_a.py', lock_config=False)
...
def main(_):
    ...
    for i in tqdm.tqdm(range(1, FLAGS.offline_steps + FLAGS.online_steps + 1), smoothing=0.1, dynamic_ncols=True):
        if i <= FLAGS.offline_steps:
            ...
            action = agent.act(observations=ob, temperature=1, seed=key)
